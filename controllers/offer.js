...
export const createOffer = async (req, res) => {
    ...
    try {
        ...
        return res.status(400).json({ success: 0, message: (err.name === 'MongoServerError' && err.code === 11000) ? "Offer Title Should be Unique, This Title is already in use." : err.message, data: null })
    }
};
...
export const deleteOffer = async (req, res) => {
    ...
    try {
        if (mongoose.isValidObjectId(req.params.id)) // checking if the user passed ID is valid
        {
            ...
            if (tempoffer == null) {
                return res.status(404).json({ success: 0, message: "No offer with this ID available." });
            }
            else {
            ...
            return res.status(400).json({ succes: 0, message: "Invalid Offer-ID.", data: null });
        }
    }
    catch (err) {
...
export const getOfferByDate = async (req, res) => {
    try {
        ...
        if (isDate(req_date) || req_date.toLowerCase() == "today") {
            ...
            return res.status(400).json({ message: "Enter Valid Date in YYYY-MM-DD format." });
        }
    }
    catch (err) {
        return res.status(500).json({ success: 0, message: err.message, data: null });
...
export const updateOffer = async (req, res) => {
    try {
        ...
        if (err.name === 'MongoServerError' && err.code === 11000) {
            return res.status(406).json({ success: 0, message: "Offer Title Should be Unique, This Title is already in use." });
        }
        else {
            return res.status(500).json({ success: 0, message: err.message, data: null });
        }
